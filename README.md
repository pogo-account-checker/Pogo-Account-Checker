...
You can use MAD's PogoDroid to get additional information, like player info, Pokémon data, and inventory item data. PogoDroid is a MITM app used to extract data from PoGo for maps. In order to use PogoDroid with PAC you have to enable `Use PogoDroid` in PAC's settings and enable `Send raw data (base64 encode)` + set `RAW POST Destination` to `http://127.0.0.1:8080` in PogoDroid's settings. You can change the port to something other than `8080` if it's not open on your device. Make sure that PogoDroid injects before an account is logged in by setting `Injection delay` to a low value, 10 should be fine. **It's VERY important to disable all PogoDroid notifications because they might interfere with PAC's screen detection.** You can more info about PogoDroid here: https://mad-docs.readthedocs.io/en/latest/device-setup/
